...
describe('send button operations', function () {
    test('send button permission', function () {
        $application = Application::factory()->create();
        $user = User::factory()->create();
        $route = 'application.sendButton';
        $permission = 'application.update';
        assertFalse($user->can($permission));

        $this->actingAs($user)
            ->get(route($route, $application))
            ->assertForbidden();
    ...
    test('can send application embed', function () {
        Http::fake();
        $application = Application::factory()->withEmbed()->create();
        $user = User::factory()->owner()->create();
        $response = $this->actingAs($user)
            ->get(route('application.sendButton', $application))
            ->assertOk();

        assertEquals(1, $response->getContent());

        Http::assertSent(fn ($request) => $request->data()['embeds'][0]->title === $application->embed_title &&
            $request->data()['embeds'][0]->description === $application->embed_description &&
            $request->data()['components'][0]->components[0]->style === $application->embed_button_color
        );
    });

    test('can not send application embed with missing embed attribute', function (string $attribute) {
        Http::fake();
        $application = Application::factory()->create([
            $attribute => null,
        ]);
