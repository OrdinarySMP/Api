...
describe('history operations', function () {
    test('history permission', function () {
        $permission = 'applicationSubmission.read';
        $route = 'application-submission.history';
        ...
        $this->actingAs($user)
            ->get(route($route, $applicationSubmission))
            ->assertForbidden();
    ...
    test('can read submission history', function () {
        $user = User::factory()->owner()->create();
        $applicationSubmission1 = ApplicationSubmission::factory()->create();
        $applicationSubmission2 = ApplicationSubmission::factory()->create(['discord_id' => $applicationSubmission1->discord_id]);
        $applicationSubmission3 = ApplicationSubmission::factory()->create();

        $this->actingAs($user)
            ->get(route('application-submission.history', $applicationSubmission1))
            ->assertOk()
            ->assertJsonCount(2, 'data')
            ->assertJsonPath('data.0.id', $applicationSubmission1->id)
            ->assertJsonPath('data.1.id', $applicationSubmission2->id)
            ->assertJsonMissing(['id' => $applicationSubmission3->id]);
